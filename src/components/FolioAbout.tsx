  ...
  return (
    <section id="about" className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      <motion.div
        ...
        className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-20 p-4 sm:p-5 rounded-2xl glass-pill"
      >
        <span className="text-xs font-sans text-slate-400 font-medium px-2 flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-[#FF4D2D]"></span>
          Specialized Platforms &amp; Production Stack
        </span>

        <div className="flex flex-wrap items-center gap-5 sm:gap-7 px-2 font-sans text-sm text-slate-300">
          {TRUSTED_BRANDS.map((brand) => (
            <div key={brand.name} className="flex items-center gap-2 hover:text-white transition-colors cursor-default">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center mb-16">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
            ...
            <button
              onClick={onContact}
              className="coral-btn inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-semibold uppercase tracking-wider font-sans group self-start sm:self-auto"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {previewProjects.map((proj) => (
          <motion.div
            ...
            className="group relative rounded-2xl overflow-hidden glass-card cursor-pointer border border-white/10"
          >
            {/* Tag Badge */}
            <div className="absolute top-4 right-4 z-20">
              <span className="px-3 py-1 rounded-full text-[10px] font-sans font-semibold uppercase tracking-wider bg-black/70 text-white backdrop-blur-md border border-white/10">
                {proj.badge}
              </span>
            </div>
